...
\end{table}
```
All performance measures exhibit the expected behavior. As the number of predictors increases in the model, R-squared improves, and more complex models yield lower Bayesian Information Criterion (BIC). The training Root Mean Squared Error (RMSE) consistently decreases, and the test RMSE initially decreases, eventually demonstrating a gradual increase. The choosen model is the one with the lowest BIC because it is the most parsimonious model that explains the data well.
...
![Variable importance plot](graphs/feature_import_plot.png){fig-align="center" width="523"}
